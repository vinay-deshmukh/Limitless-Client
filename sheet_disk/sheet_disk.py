...
def authorize_oauth(json_string):
    global gc
    ...
    try:
        ...
        print('ValueError: ',  v)
        raise ValueError('given json_string is not valid JSON')
...
def upload(user_file, json_file=None, email_list=None):
    ...
    with SheetUpload(
            ...
            json_file=json_file,
            email_list=email_list) as sheet:
        sheet.start_upload()
...
def main(oauth_json_string, raw_args=None, email_list=None):
    ...
    # AUTHORIZE OAUTH JSON STRING
    authorize_oauth(oauth_json_string)
    ...
    if dargs['action'] == 'upload':
        ...
        upload(up_file, up_json, email_list=email_list)
